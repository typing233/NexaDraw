...
function getElementColor(element, themeConfig) {
  if (!themeConfig) return element.color || DEFAULT_STYLES.color;
  if (element._theme && themeConfig.colors) {
    return themeConfig.colors.primary;
  }
  return element.color || DEFAULT_STYLES.color;
}

function getStrokeWidth(element, themeConfig) {
  if (!themeConfig) return element.strokeWidth || DEFAULT_STYLES.strokeWidth;
  if (element._theme && themeConfig.strokes) {
    return themeConfig.strokes.width || element.strokeWidth;
  }
  return element.strokeWidth || DEFAULT_STYLES.strokeWidth;
}

function getRoundedness(element, themeConfig) {
  if (!themeConfig || !themeConfig.effects) return 4;
  return themeConfig.effects.roundedness ?? 4;
}

function applyJitter(value, amount, seed) {
  if (!amount) return value;
  const random = Math.sin(seed) * 10000 - Math.floor(Math.sin(seed) * 10000);
  return value + (random - 0.5) * amount * 2;
}

function renderElement(element, isSelected, themeConfig) {
  const selectionStyle = isSelected ? {
    filter: 'drop-shadow(0 0 3px rgba(79, 70, 229, 0.8))'
  } : {};

  const color = getElementColor(element, themeConfig);
  const strokeWidth = getStrokeWidth(element, themeConfig);
  const roundedness = getRoundedness(element, themeConfig);
  
  const handwriting = themeConfig?.effects?.handwriting;
  const jitterAmount = handwriting ? (themeConfig.effects.jitterAmount || 1) : 0;

  const themeEffects = themeConfig?.effects || {};
  let themeStyle = { ...selectionStyle };
  
  if (themeEffects.shadow && themeConfig.colors?.primary) {
    const shadowColor = themeEffects.shadowColor || 'rgba(0, 0, 0, 0.2)';
    const shadowBlur = themeEffects.shadowBlur || 5;
    themeStyle.filter = `drop-shadow(0 0 ${shadowBlur}px ${shadowColor})`;
  }
  
  if (themeEffects.glow && themeConfig.colors?.primary) {
    const glowColor = themeEffects.glowColor || themeConfig.colors.primary;
    const glowBlur = themeEffects.glowBlur || 5;
    if (themeStyle.filter) {
      themeStyle.filter += ` drop-shadow(0 0 ${glowBlur}px ${glowColor})`;
    } else {
      themeStyle.filter = `drop-shadow(0 0 ${glowBlur}px ${glowColor})`;
    }
  }
  
  const dasharray = themeConfig?.strokes?.dasharray;

  switch (element.type) {
    case 'pencil': {
      const pencilConfig = themeConfig?.pencil || {
        size: 2,
        thinning: 0.5,
        smoothing: 0.5,
        streamline: 0.5
      ...
      const stroke = getStroke(element.points, {
        ...
        thinning: pencilConfig.thinning,
        smoothing: pencilConfig.smoothing,
        streamline: pencilConfig.streamline
      });
    ...
    case 'rectangle': {
      let x = element.width < 0 ? element.x + element.width : element.x;
      let y = element.height < 0 ? element.y + element.height : element.y;
      const width = Math.abs(element.width);
      const height = Math.abs(element.height);
      
      if (jitterAmount) {
        x = applyJitter(x, jitterAmount, element.id.length);
        y = applyJitter(y, jitterAmount, element.id.length + 1);
      }
    ...
    case 'circle': {
      ...
      if (jitterAmount) {
        cx = applyJitter(cx, jitterAmount, element.id.length);
        cy = applyJitter(cy, jitterAmount, element.id.length + 1);
      }
    ...
    case 'arrow': {
      ...
      const arrowLength = 12 + strokeWidth * 2;
...
function Canvas({
  ...
  const themeColors = themeConfig?.colors || { background: '#fafafa', grid: '#dddddd' };
  ...
  const handlePointerDown = useCallback((e) => {
    ...
    if (currentTool === 'text') {
      const newElement = createNewElement(currentTool, point, themeConfig);
    ...
    e.currentTarget.setPointerCapture(e.pointerId);
      ...
      lastClickTime, lastClickedElementId, onSelectedElementChange, themeConfig]);
  ...
  const renderMagicCursor = (uid, pos, collaborator) => {
    const userColor = getUserColor(uid);
    const userName = collaborator?.name || '未知用户';
    ...
    return (
      <g key={uid} style={{ pointerEvents: 'none' }}>
        ...
        />
        <path
          d={`M ${pos.x} ${pos.y} L ${pos.x + 14} ${pos.y - 8} L ${pos.x + 10} ${pos.y} L ${pos.x + 8} ${pos.y + 10} Z`}
          fill="white"
          stroke={userColor}
          strokeWidth="1.5"
        />
        <g transform={`translate(${pos.x + 18}, ${pos.y - 2})`}>
          <rect
            x="0"
            y="-12"
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage: `
          radial-gradient(circle, ${themeColors.grid} 1px, transparent 1px)
        `,
      ...
      onWheel={handleWheel}
    >
      {themeConfig?.effects?.paperTexture && (
        <div
          style={{
            position: 'absolute',
            top: 0,
            ...
            width: '100%',
            height: '100%',
            backgroundColor: themeConfig.effects.paperTextureColor || 'rgba(139, 115, 85, 0.02)',
            pointerEvents: 'none',
            zIndex: 1
          }}
        />
      ...
      >
        <g transform={`translate(${camera.x}, ${camera.y}) scale(${camera.zoom})`}>
          {elements.map(element => (
            <g key={element.id}>
              {renderElement(element, selectedElementId === element.id, themeConfig)}
            </g>
          ))}
          
          {currentElement && (
            <g>
              {renderElement(currentElement, false, themeConfig)}
            </g>
          )}
          ...
          {Object.entries(cursorPositions).map(([uid, pos]) => {
            ...
            return renderMagicCursor(uid, pos, collaborator);
          })}
        </g>
      </svg>
...
function createNewElement(tool, point, themeConfig) {
  const id = uuidv4();
  const timestamp = Date.now();
  const themeStrokes = themeConfig?.strokes || { width: 2 };
  const themeColors = themeConfig?.colors || { primary: COLORS.black };
  ...
  switch (tool) {
    case 'pencil':
      return {
        ...
        strokeWidth: themeStrokes.width,
